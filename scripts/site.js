Y.use('node', function (Y) {
	...
	Y.namespace('Template').Site = Singleton.create({
		...
		init: function() {
			...
			this.insertHeaderHRs();


		},

		insertHeaderHRs:function(){
			Y.all('.index-page .page-content h2')
				.each(function (h2) {
					h2.insert('<div class="hr-short"></div>', 'after');
				});
		},
		...
		syncUI: function () {
			...
			// SLK: removing nav center logo/title
			//this.runCenterNav();
		...
		scrollNav: function () {
			...
			if (Y.one('.collection-type-index.homepage') && Y.one('#header #mainNavWrapper') && Y.one('.index.home')) {
				...
				var indexSection = Y.all('.index-section');
		...
		transparentHeaderPadding: function () {
			...
			var extraPadding = parseInt(Y.one('#header .header-inner').getComputedStyle('paddingTop'), 10) / 2;
